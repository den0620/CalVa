import discord,os,asyncio,time,random,ffmpeg,aiohttp,aiofiles,base64,copy,requests#,pyttsx3
...
client = discord.Bot(debug_guilds=[981625388503531621,929440411758518302,758316954087456789,856154142183784448,884789756741951549],intents=discord.Intents.all())
...
MusicCommands=client.create_group('музыка','Команды для музыки')
...
# ===
@client.command(name_localizations={'en-US': 'deafen', 'ru': 'заглушить'},description_localizations={'en-US': 'Deafen bot (self)', 'ru': 'Самозаглушить бота'})
async def заглушить(message):
...
"{PREF}музыка включи [URL]" - Включить звук с видео по URL YouTube
"{PREF}музыка включи (без url)" - Включу следующее в очереди (как скип)
"{PREF}музыка выключи" - Выключу играющую музыку
"{PREF}музыка пауза" - Поставлю играющую музыку на паузу
"{PREF}музыка продолжи" - Сниму играющую музыку с паузы
"{PREF}музыка выйди" - Выйду из ГС и выключу играющую музыку
"{PREF}музыка очередь" - Добавлю видео в очередь
"{PREF}музыка повтор" - Включу/Выключу повтор играющей музыки''', inline=False)
        embed.set_footer(text='Музыка может работать с перебоями после 6 часов использования => надо выгнать и пригласить бота обратно в голосовой канал')
...
async def рандом(message,от: discord.Option(int,description='Целое число'),до: discord.Option(int,description='Целое число')): await message.respond(f'```{random.randint(*sorted([от,до]))}```')
# ===============QUESTION===============
...
async def ChangeImgPage(msg,Img,Prev,Next,PostId):
    ...
    embed.set_image(url=Img)
    await msg.edit_original_message(content=None,embed=embed,view=view)
@client.command(description='Поиск по danbooru')
async def пикча(message,категория: discord.Option(str,description='Поиск по тегу',required=False),айди: discord.Option(str,description='Поиск по айди',required=False)):
    Category=категория
    MessagePostId=айди
    if True:
...
async def ConnectToVoiceChannel(message):
    try:
        ...
        try:
            ...
            return('NotConnected')
# ===== MAIN =====
@MusicCommands.command(description='Включу музыку с видео')
    ...
    if ссылка==None:
        ...
        if ConnectResult!='NotConnected':
            ...
            try:
                ...
                return
# ===============QUEUE COMMAND===============
@MusicCommands.command(description='Добавлю трек в очередь')
async def очередь(message,ссылка: discord.Option(str,description='Ссылка на видео')):
    try:
        ...
        return
# ===============LEAVE COMMAND===============
@MusicCommands.command(description='Выйду из голосового канала')
async def выйди(message):
    ...
    try:
        ...
        return
# ===============PAUSE COMMAND===============
@MusicCommands.command(description='Приостановлю воспроизведение')
async def пауза(message):
    ...
    try:
        ...
        return
# ===============RESUME COMMAND===============
@MusicCommands.command(description='Продолжу воспроизведение')
async def продолжи(message):
    ...
    try:
        ...
        return
# ===============STOP COMMAND===============
@MusicCommands.command(description='Выключу музыку')
...
# ===============LOOP COMMAND==============
@MusicCommands.command(description='Включу/Выключу повтор играющего трека')
...
# ===============CalVaTTS===============
@client.command(description='Text To Speech')
async def cvtts(message,текст: discord.Option(str,description='Текст'),язык: discord.Option(str,description='Язык по IETF (по умолчанию "ru")')='ru'):
    if True: #message.author.id in adminlist
        if await ConnectToVoiceChannel(message)!='NotConnected':
            text=текст; lang=язык
            Voice=discord.utils.get(client.voice_clients,guild=message.guild)
            ...
            tts.save(f'CVTTS{msg.id}.mp3')
            #engine.save_to_file(text,f'CVTTS{msg.id}.mp3')
            #engine.runAndWait()
            Voice.play(discord.FFmpegPCMAudio(f'CVTTS{msg.id}.mp3'))
            while Voice.is_playing() or Voice.is_paused():
                await asyncio.sleep(1)
            await msg.edit_original_message(content=f'```"{text}" (выполнено)```')
            os.remove(f'CVTTS{msg.id}.mp3')
        else:
            pass
    else:
...
async def on_message(message):
        #374558482525061122 = VoDKa#6480
        if message.author.id==374558482525061122:
            for atm in message.attachments:
                try:
                        atm=str(atm)
                        name=atm.split('/')[-1]
                        response = requests.get(atm)
                        image = Image.open(BytesIO(response.content))
                        image.save(f"{os.path.realpath(__file__)[:os.path.realpath(__file__).rfind('/')+1]}SILVER_LOG/{name}")
                except:
                    pass
