    ...
    if message.content.startswith(PREF):
        if message.content.startswith(str(PREF)+'сервера') and str(message.author)=='den0620#5150':
            ...
            try:
                ...
                embed.set_footer(text="Подробнее о получении Social Credits ~SC help")
...
''')
            ...
            embed.add_field(name="Награды:", value="От 20.000 Социальных кредитов\n-Статус ГИПЕР НАТУРАЛ\n-На вас не действует команда ~кто", inline=False)
            await message.channel.send(embed=embed)
...
''')
            ...
            embed.add_field(name="~play [URL] / ~включи [URL]", value="Включить звук с видео по URL YouTube", inline=False)
            embed.add_field(name="~stop / ~выключи", value="Выключу играющую музыку", inline=False)
            embed.add_field(name="~pause / ~пауза", value="Поставлю играющую музыку на паузу", inline=False)
            embed.add_field(name="~resume / ~продолжи", value="Сниму играющую музыку с паузы", inline=False)
            embed.add_field(name="~leave / ~выйди", value="Выйду из ГС и выключу играющую музыку", inline=False)
            embed.add_field(name="#НЕРАБОТАЕТ ~queue [URL] / ~очередь [URL]", value="Добавлю видео в очередь",inline=False)
            embed.add_field(name="#НЕРАБОТАЕТ ~skip / ~пропусти", value="Пропущу играющую музыку", inline=False)
            embed.add_field(name="#НЕРАБОТАЕТ ~loop / ~повтор", value="Включу/Выключу повтор играющей музыки", inline=False)
            #                        /|\
...
''')
            ...
            embed=discord.Embed(title='[Добавить бота]',url='https://discord.com/api/oauth2/authorize?client_id=968795473068560395&permissions=0&scope=bot',description='"~" - действительная приставка',color=0x3ae47b)
